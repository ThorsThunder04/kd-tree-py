...
def k_dist(pt1: tuple[float,...], pt2: tuple[float,...], k: int) -> float:
    """Calculates ecludian distance between two points in the kth dimension
    ...
    """
    assert len(pt1) == len(pt2) and len(pt2) == k, "Incompatible point dimensions for given dimension k"
...
def find_NN(point: tuple[float,...], tree: (Node|None), _mindist: float = -1, _depth: int = 0) -> tuple[float,...] :
    ...
    if point[axis] < tree.val[axis] and tree.left is not None:
        ...
        if this_mindist < _mindist:
           _mindist = this_mindist
